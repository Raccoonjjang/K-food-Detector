...
public class GraphicOverlay extends ViewGroup {
  ...
  public void add(Graphic graphic) {
    ...
    postInvalidate(); // 새 그래픽이 추가되면 뷰를 다시 그리기
  }
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    // 자식 뷰의 위치를 설정하지 않고, 사용자가 그린 그래픽만 사용하기 때문에 빈 메서드로 구현합니다.
    // 필요한 경우 자식 뷰를 레이아웃하는 코드를 추가할 수 있습니다.
  }
  ...
  public abstract static class Graphic {
    ...
    protected float translateX(float x) {
      return x * overlay.getWidth() / overlay.getWidth(); // Adjust according to the overlay width
    }

    protected float translateY(float y) {
      return y * overlay.getHeight() / overlay.getHeight(); // Adjust according to the overlay height
    }
